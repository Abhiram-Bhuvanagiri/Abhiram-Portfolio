<template>
  <div v-if="isVisible" class="intro-loader" aria-hidden="true">
    ...
    <div class="intro-title" :class="{ 'is-ready': isTitleReady }" ref="titleEl">
      <div class="title-row">
        <div class="title-charts-cont"><span ref="seg1">ABHI</span></div>
        <div class="title-charts-cont"><span ref="seg2">RAM</span></div>
      </div>
      <div class="title-row title-row-2">
        <div class="title-charts-cont"><span ref="seg3">BHUVA</span></div>
        <div class="title-charts-cont"><span ref="seg4">NAGIRI</span></div>
      </div>
    </div>
  </div>
</template>
...
const runSlideAnimation = () => {
  ...
  // Wave 1 — outer segments (ABHI slides in from right, NAGIRI from left)
  slideTimeline
    .fromTo(
      seg1.value,
      { x: "-3em" },
    ...
    // Wave 2 — inner segments (RAM slides in from left, BHUVA from right)
    .fromTo(
      seg2.value,
      { x: "3em" },
      { x: "0em", duration: slideDuration, ease: "power2.out" },
